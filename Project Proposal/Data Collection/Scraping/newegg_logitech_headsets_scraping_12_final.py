...
for page_num in range(1, 4):
    ...
    headers = {
        ...
        "Referer": f"https://www.newegg.com/p/pl?d=logitech+gaming+headset&PageSize=96&page={page_num-1}" if page_num > 1 else "https://www.newegg.com/p/pl?d=logitech+gaming+headset&PageSize=96&page=1",
        ...
        if response.status_code == 200:
            ...
            for product in products:  
                ...
                if link_tag:
                    ...
                    if "refurbished" in title.lower() or "replacement" in title.lower() or "used" in title.lower() or "ablegrid" in title.lower() or "bundle" in title.lower() or "3-in-1" in title.lower() or "stand" in title.lower():
                        print(f"Skipping: {title} (Refurbished/Replacement/Used)")
                        continue
...
with open('logitech_product_links.json', 'w') as f:
...
for index, product in enumerate(product_data, start=1): 
    ...
    try: 
        ...
        if response.status_code == 200:
            ...
            if index % 10 == 0:
                with open('logitech_product_data.json', 'w', encoding="utf-8") as f:
